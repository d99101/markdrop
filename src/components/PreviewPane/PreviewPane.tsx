...
function buildMarkdownComponents(
  ...
  return {
    ...
    td: ({ children }) => (
      <td style={{ border: `1px solid ${t.border}`, padding: '0.5rem 0.75rem', color: t.text }}>
        {children}
      </td>
    ),
    p: ({ children }) => <p style={{ color: t.text }}>{children}</p>,
    h1: ({ children }) => (
      <h1 style={{ color: t.text, borderBottom: `1px solid ${t.border}`, paddingBottom: '0.3rem' }}>
        {children}
      </h1>
    ),
    h2: ({ children }) => (
      <h2 style={{ color: t.text, borderBottom: `1px solid ${t.border}`, paddingBottom: '0.2rem' }}>
        {children}
      </h2>
    ),
    h3: ({ children }) => <h3 style={{ color: t.text }}>{children}</h3>,
    a: ({ children, href }) => (
      <a href={href} style={{ color: t.resolved === 'dark' ? '#58a6ff' : '#0969da' }}>
        {children}
      </a>
    ),
    blockquote: ({ children }) => (
    ...
    hr: () => (
      <hr style={{ border: 'none', borderTop: `1px solid ${t.border}`, margin: '1.5rem 0' }} />
    ),
