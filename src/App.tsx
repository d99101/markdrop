...
function App() {
  ...
  const [emptyHint, setEmptyHint] = useState<'hidden' | 'visible' | 'fading'>('hidden')
  ...
  const handleDownload = () => {
    if (!content?.trim()) {
      setEmptyHint('visible')
      setTimeout(() => setEmptyHint('fading'), 2000)
      setTimeout(() => setEmptyHint('hidden'), 2700)
      return
    }
    ...
    <div onDrop={onDrop} onDragOver={onDragOver} onDragLeave={onDragLeave} style={{
      ...
      {emptyHint !== 'hidden' && (
        <div style={{
          ...
          opacity: emptyHint === 'visible' ? 1 : 0, transition: 'opacity 0.6s ease',
